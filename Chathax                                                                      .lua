...
TextLabel4.Size = UDim2.new(1, 0, 0, 21)
...
TextButton11.MouseButton1Click:Connect(function()
  game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("" .. TextBox3.Text .. (" "):rep(147) .. "[" .. TextBox8.Text .. "]: " .. TextBox10.Text .. "", "All")
end)
function dragify(a)
  dragToggle = nil
  dragInput = nil
  ...
  function updateInput(a)
    game:GetService("TweenService"):Create(va, TweenInfo.new(0.3), {
      Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + a.Position - dragStart.X, startPos.Y.Scale, startPos.Y.Offset + a.Position - dragStart.Y)
    }):Play()
  end
  a.InputBegan:Connect(function(a)
    if a.UserInputType ~= Enum.UserInputType.MouseButton1 then
    end
    if a.UserInputType == Enum.UserInputType.Touch and va:GetFocusedTextBox() == nil then
      dragToggle = true
      dragStart = a.Position
      startPos = vb.Position
...
dragify(Frame1)
for fk, fl in pairs(mas:GetChildren()) do
  fl.Parent = game.CoreGui
  pcall(function()
    va:MakeJoints()
  end)
end
mas:Destroy()
for fk, fl in pairs(cors) do
  spawn(function()
    pcall(va)
  end)
end
